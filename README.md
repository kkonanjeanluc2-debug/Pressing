...
- 💳 Abonnements CinetPay : Gratuit (20 tickets/mois) · Pro 5 000 FCFA · Réseau 12 000 FCFA,
...
- Comptes [CinetPay](https://cinetpay.com) et [Orange Developer](https://developer.orange.com) (optionnels en dev)
...
| `CINETPAY_API_KEY` / `CINETPAY_SITE_ID` / `CINETPAY_WEBHOOK_SECRET` | Tableau de bord CinetPay |
...
npx supabase functions deploy cinetpay-webhook --no-verify-jwt
...
> Les routes Next.js `app/api/sms` et `app/api/cinetpay-webhook` couvrent les
...
- Webhook CinetPay : signature **HMAC SHA-256** + re-vérification de la
  transaction auprès de l'API CinetPay + idempotence
